...
class Vinmart:
    ...
    def choose_location(self):
        ...
        try:
            wait = WebDriverWait(self.BROWSER, 60).until(EC.element_to_be_clickable((By.XPATH, "//span[contains(text(),'TP. Hà Nội')]")))
        except TimeoutException:
            pass
        sleep(2)
        # Choose city
        city = self.BROWSER.find_element_by_xpath("//span[contains(text(),'TP. Hà Nội')]").click() # Hà Nội
        sleep(2)
        # Choose district
        district = self.BROWSER.find_element_by_xpath("//span[contains(text(),'Q. Hai Bà Trưng')]").click() # Hai Bà Trưng
        sleep(2)
        # Choose ward
        ward = self.BROWSER.find_element_by_xpath("//span[contains(text(),'P. Minh Khai')]").click() # Minh Khai
    ...
    def scrap_data(self, cat):
        ...
        try:
            ...
            list = soup.find_all('div', {'class':'product-card-two__Card-sc-1lvbgq2-0 EvJvz product-card'})
            log.info('Found ' + str(len(list)) + ' products')
            for item in list:
                row = {}
                # Product name
                if item.find('h2', {'class':'product-card-two__Title-sc-1lvbgq2-4 dokUNv'}) != None:
                    good_name = item.find('h2', {'class':'product-card-two__Title-sc-1lvbgq2-4 dokUNv'}).text.strip()
                    row['good_name'] = good_name
                else:
                    None
                # Price
                if item.find('div', {'class':'product-card-two__Price-sc-1lvbgq2-6 bMEYxB'}) != None:
                    price = item.find('div', {'class':'product-card-two__Price-sc-1lvbgq2-6 bMEYxB'}).text.strip()
                    price = price.split('₫')[0]
                    price = price.strip()
                    row['price'] = price
                else:
                ...
                if item.find('div', {'class':'product-card-two__SalePrice-sc-1lvbgq2-7 iZguIh'}) != None:
                    old_price = item.find('div', {'class':'product-card-two__SalePrice-sc-1lvbgq2-7 iZguIh'}).text.strip()
