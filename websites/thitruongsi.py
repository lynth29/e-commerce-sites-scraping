...
class ThiTruongSi:
    ...
    def get_category_list(self):
        ...
        cat_l1_dict = self.session.get('https://thitruongsi.com/endpoint/v2/product/categories.json?level=1').json()
        ...
        cat_l2_dict = self.session.get('https://thitruongsi.com/endpoint/v2/product/categories.json?level=2').json()
        ...
        for cat in cat_l2_list:
            ...
            try:
                ...
                try:
                    next_page["cat_l1"] = cat_l1_bar[str(cat_l2_parent[str(cat['p_id'])])]
                except KeyError:
                    next_page["cat_l1"] = ""
    ...
    def scrap_data(self, cat):
        ...
        # Find page number
        total_prods = soup.find('div', class_='bg-fff rounded p-2 mb-2').find('strong').text.strip()
        page_num = int(int(total_prods) / 40 + 1)
        # Get all products
        items_list = []
        for i in range(1, page_num+1):
            if i > 1:
                res = self.session.get(f"{self.BASE_URL}/category/child/{cat['href']}.html/?&limit=40&offset={40*(i-1)}&page={i}")
                soup = BeautifulSoup(res.content, "lxml")
        ...
        for item in items_list:
            try:
                ...
                if prod_soup.find('div', class_='jsx-1704392005 multiple-prices') != None:
                    multi_price_section = prod_soup.find('div', class_='jsx-1704392005 multiple-prices').find_all('div')
                    price_1 = int(multi_price_section[1].text.strip().split('đ')[0].replace(',','').replace('.','').replace('x','0'))
                    price_2 = int(multi_price_section[4].text.strip().split('đ')[0].replace(',','').replace('.','').replace('x','0'))
                    row["price_2"] = price_2
                    if len(multi_price_section) > 7:
                        price_3 = int(multi_price_section[7].text.strip().split('đ')[0].replace(',','').replace('.','').replace('x','0'))
                        row["price_3"] = price_3
                    else:
                        row["price_3"] = ""
                else:
                    price_1 = int(price_section.find('span').text.strip().split('đ')[0].replace(',','').replace('x','0').replace('.',''))
                    row["price_2"] = ""
                    row["price_3"] = ""
                row["price_1"] = price_1
                # Get shop information
                shop_section = prod_soup.find('div', class_='border rounded css-1il6ewt')
                shop_name = shop_section.find('a').text.strip()
                row["shop_name"] = shop_name
                shop_href = shop_section.find('a')['href']
                row["shop_href"] = shop_href
                shop_address = shop_section.find('div', class_='css-16yk86w').text.strip()
                ...
                try:
                    shop_rep_rate = [int(shop_stats[id+1].text.strip().split('%')[0])/100 for id, i in enumerate(shop_stats) if i.text.strip() == 'Tỷ lệ phản hồi'][0]
                except IndexError:
                    shop_rep_rate = 0
                try: 
                    shop_followers = [int(shop_stats[id+1].text.strip().replace(',','')) for id, i in enumerate(shop_stats) if i.text.strip() == 'Người theo dõi'][0]
                except IndexError:
                    shop_followers = 0
